...
(function($) {
	...
	$.fileManager = function(opt) {
		...
		//设置初始值
		if (opt.host == '') { //host为必须设置的值
			jQuery.error('请按照如下要求设置host的值：host: \'<@url value="/{path}" />\'');
		}
		opt = $.extend(true, {
			host: '',
			uploadUrl : opt.host.replace('{path}', 'file/upload'), //如果没有设置上传文件的地址话，默认使用自带的上传地址
			manager : true //是否进入 文件管理界面，默认为true
			,callback: function(){} //空回调
		}, opt);
		
		
		var filesManager = $('<div id="filesManager"></div>').appendTo($('body'));
		...
		if (opt.manager) {
			filesManager.dialog({
				...
				minimizable: false,
				modal: true,
				href : opt.host.replace('{path}', 'file/manager'),
				buttons:[
				         {text:'确定',handler:function() {
				        	 filesManager.dialog('destroy');
				         }},
				...
				toolbar : [
				         {id: 'uploadify', text: '上传文件', iconCls:'icon-add'},
				         {text: '删除文件', iconCls:'icon-cancel'}
				]
			});
			
			$("#uploadify").uploadify({
                'uploader': opt.host.replace('{path}', 'file/uploadifyswf'),
			...
			filesManager.dialog({
				...
				cache: false,
				modal: true,
				content: '<input id="fileupload" style="width:100%;height:27px;padding:0px;margin:0px;" type="file" name="fileupload">',
				buttons:[
				         {text:'上传',handler:function() {
				        	 if ($('#fileupload', filesManager).val() == '') {
				        		 alert('请选择文件后再进行上传操作');
				        		 return;
				        	 }
				        	 $.ajaxFileUpload({
